...
export async function create(identifier, purpose) {
...
export async function verify(identifier, inputCode, purpose) {
    const otpKey = `otp:${purpose}:${identifier}:code`;
    const attemptsKey = `otp:${purpose}:${identifier}:attempts`;
    ...
    if (attempts <= 0) {
        await invalidate(identifier, purpose);
    ...
    const hashedToken = hashCode(resetToken);
...
export async function isVerified(identifier, inputToken = null, purpose) {
    if (inputToken) {
        const savedToken = await redis.get(`otp:${purpose}:${identifier}:token`);
        if (!savedToken) return false;

        return savedToken === hashCode(inputToken);
    } else {
...
export async function clearVerified(identifier, purpose) {
    await redis.del(`otp:${purpose}:${identifier}:token`);
    await redis.del(`otp:${purpose}:${identifier}:verified`);
}

export async function invalidate(identifier, purpose) {
    await redis.del(
...
export async function getInfo(identifier, purpose) {
    const otpKey = `otp:${purpose}:${identifier}:code`;
    const attemptsKey = `otp:${purpose}:${identifier}:attempts`;
