...
function DeletedUsers() {
    ...
    const fetchData = useCallback(async () => {
        const res = await axiosInstance.get("/api/users/deleted", {
            params: {
                limit,
                cursor,
                direction,
    ...
    useEffect(() => {
        ...
        socket.on("userPermanentlyDeleted", refresh);

        return () => {
            socket.off("userDeleted", refresh);
            socket.off("userRestored", refresh);
            socket.off("userPermanentlyDeleted", refresh);
        };
    }, [socket, fetchData]);
    ...
    const handleRestore = async (id) => {
        setRestoring(id);
        try {
            await axiosInstance.post(`/api/users/restore/${id}`);
            fetchData();
        } finally {
            setRestoring(null);
        }
    ...
    const handlePermanentDelete = async (id) => {
        ...
        setDeleting(id);
        try {
            await axiosInstance.delete(`/api/users/deleted/${id}`);
            fetchData();
        } finally {
            setDeleting(null);
        }
