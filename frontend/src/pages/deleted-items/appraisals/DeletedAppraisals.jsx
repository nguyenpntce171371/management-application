...
function DeletedAppraisals() {
    ...
    const fetchData = useCallback(async () => {
        const res = await axiosInstance.get("/api/appraisals/deleted", {
            params: {
                limit,
                cursor,
                direction,
    ...
    useEffect(() => {
        ...
        socket.on("appraisalPermanentlyDeleted", refresh);

        return () => {
            socket.off("appraisalDeleted", refresh);
            socket.off("appraisalRestored", refresh);
            socket.off("appraisalPermanentlyDeleted", refresh);
        };
    }, [socket, fetchData]);
    ...
    const handleRestore = async (id) => {
        setRestoring(id);
        try {
            await axiosInstance.post(`/api/appraisals/restore/${id}`);
            fetchData();
        } finally {
            setRestoring(null);
        }
    ...
    const handlePermanentDelete = async (id) => {
        ...
        setDeleting(id);
        try {
            await axiosInstance.delete(`/api/appraisals/deleted/${id}`);
            fetchData();
        } finally {
            setDeleting(null);
        }
...
export default DeletedAppraisals;